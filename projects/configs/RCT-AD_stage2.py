...
# ================ RCT-AD: Reliable Context Awareness ================
# Reliability threshold tau, bounded meta-update budget K_r, FILO/memory
# capacity K, and reliability-score coefficients (alpha1..alpha5) for
# (IoU, Conf, 1-H, S, P). See Section III-B and Table VIII/IX.
use_rca = True
rca_tau = 0.85
rca_max_meta_iters = 3
rca_queue_length = 6
rca_alphas = (0.25, 0.30, 0.15, 0.20, 0.10)
rca_beta = 0.2
...
model = dict(
    ...
    rca=dict(
        type="ReliableContextAwareness",
        embed_dims=embed_dims,
        tau=rca_tau,
        max_meta_iters=rca_max_meta_iters,
        queue_length=rca_queue_length,
        alphas=rca_alphas,
        beta=rca_beta,
    ...
    head=dict(
        ...
        use_motion_for_det=use_motion_for_det,
        det_head=dict(
            type="Sparse4DHead",
        ...
        motion_plan_head=dict(
            ...
            operation_order_state_self_motion=(
                [
                    "cross_gnn",
                    "norm",
                    "cross_gnn",
            ...
            operation_order_state_self_plan=(
                [
                    "cross_gnn",
                    "norm",
                    "cross_gnn",
            ...
            temp_graph_model=dict(  # temp_gnn
                type="MultiheadAttention",
                embed_dims=embed_dims if not decouple_attn_motion else embed_dims * 2,
                num_heads=num_groups,
                batch_first=True,
            ...
            cross_graph_model=dict(  # cross_gnn
                type="MultiheadFlashAttention",
                embed_dims=embed_dims,
                num_heads=num_groups,
                batch_first=True,
            ...
            motion_loss_reg=dict(type='L1Loss', loss_weight=0.05),
